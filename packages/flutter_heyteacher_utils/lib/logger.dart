...
class _LoggerScreenState extends State<LoggerScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(
      ...
      body: StreamBuilder<Object>(
          stream: Logger.root.onRecord,
          // Displays each log message as a Text widget in a ListView.
          builder: (_, snapshot) => ListView(children: [
                // Filters logs based on the selected _filterLevel.
                ...LoggerModel.instance.logs
                    .where((logRecord) =>
                        _filterLevel == null || logRecord.level == _filterLevel)
                    .map(
...
class LoggerModel {
  ...
  /// A list to store formatted log messages.
  /// Stores the raw [LogRecord] objects.
  ...
  /// Gets the list of stored [LogRecord] objects.
  List<LogRecord> get logs => _logs;

  /// Provides the singleton instance of [LoggerModel].
  ///
  /// If an instance doesn't exist, it creates one.
  static LoggerModel get instance => _instance ??= LoggerModel._();
  ...
  configure() async {
    ...
    _loggerSubscription = Logger.root.onRecord.listen((record) {
      ...
      // Maintain a maximum of 500 logs in memory.
